...
def count_violations(new_cg, new_moi, target_cg, target_moi):
    """Counts the number of constraint violations for a given solution."""
    violations = 0
    ...
    moi_diag_error = np.abs((np.diag(new_moi) - np.diag(target_moi)) / np.diag(target_moi))
    violations += np.sum(moi_diag_error > 0.03)
...
def calculate_fitness_score(solution, initial_mass, initial_cg, initial_moi, target_cg, target_moi, ballast_coords, ballast_mass=5.0):
    """
    Calculates a continuous fitness score for a solution.
    A lower score is better. The score is the sum of squared normalized errors.
    """
    _, new_cg, new_moi = calculate_combined_properties(initial_mass, initial_cg, initial_moi, ballast_coords, solution, ballast_mass)

    def get_normalized_error(achieved, target, tolerance):
        """Calculates the squared error normalized by the tolerance."""
        if abs(target) < 1e-9:
            return 0 if abs(achieved) < 1e-9 else (achieved / tolerance)**2
        percent_error = (achieved - target) / target
        return (percent_error / tolerance)**2

    # CG errors (1% tolerance)
    cg_errors = sum(get_normalized_error(new_cg[i], target_cg[i], 0.01) for i in range(3))

    # MOI errors (3% tolerance)
    f_ixx, f_iyy, f_izz = new_moi[0,0], new_moi[1,1], new_moi[2,2]
    f_ixy, f_ixz, f_iyz = -new_moi[0,1], -new_moi[0,2], -new_moi[1,2]
    ...
    achieved_moi = [f_ixx, f_iyy, f_izz, f_ixy, f_ixz, f_iyz]
    target_moi_vals = [t_ixx, t_iyy, t_izz, t_ixy, t_ixz, t_iyz]
    moi_errors = sum(get_normalized_error(achieved_moi[i], target_moi_vals[i], 0.03) for i in range(6))

    # Give more weight to MOI errors as they are harder to satisfy
    return cg_errors + (2 * moi_errors)

def run_simulated_annealing(initial_mass, initial_cg, initial_moi, target_cg, target_moi, ballast_coords, num_ballasts_to_add, ballast_mass):
    """
    ...
    violations = -1 # Start with a non-zero violation count

    # Initial solution
    current_solution = np.zeros(len(ballast_coords), dtype=bool)
    true_indices = np.random.choice(len(ballast_coords), num_ballasts_to_add, replace=False)
    current_solution[true_indices] = True

    current_energy = calculate_fitness_score(current_solution, initial_mass, initial_cg, initial_moi, target_cg, target_moi, ballast_coords, ballast_mass)
    ...
    for i in range(num_iterations):
        # Create a neighbor solution by swapping one ballast
        neighbor_solution = current_solution.copy()
        true_indices = np.where(neighbor_solution)[0]
        false_indices = np.where(~neighbor_solution)[0]

        idx_to_remove = np.random.choice(true_indices)
        idx_to_add = np.random.choice(false_indices)

        neighbor_solution[idx_to_remove] = False
        neighbor_solution[idx_to_add] = True

        neighbor_energy = calculate_fitness_score(neighbor_solution, initial_mass, initial_cg, initial_moi, target_cg, target_moi, ballast_coords, ballast_mass)
        ...
        if neighbor_energy < current_energy:
            ...
            acceptance_prob = np.exp((current_energy - neighbor_energy) / temp)
        ...
        if (i + 1) % 2000 == 0:
            final_mass, final_cg, final_moi = calculate_combined_properties(initial_mass, initial_cg, initial_moi, ballast_coords, best_solution, ballast_mass)
            violations = count_violations(final_cg, final_moi, target_cg, target_moi)
            print(f"Iteration {i+1}, Temp: {temp:.2f}, Best Energy: {best_energy:.4f}, Violations: {violations}")

        if violations == 0:
            print(f"Feasible solution found at iteration {i+1}!")
            break
...
def print_results_breakdown(final_mass, final_cg, final_moi, target_mass, target_cg, target_moi):
    ...
    print(f"{'Property':<12} | {'Target':<25} | {'Achieved':<25} | {'Error (%)':<10}")
    print("-" * 80)
    print(f"{'Mass (kg)':<12} | {target_mass:<25.6f} | {final_mass:<25.6f} | {mass_error:<10.4f}")
    print(f"{'COG_x (mm)':<12} | {target_cg[0]:<25.4f} | {final_cg[0]:<25.4f} | {cg_x_error:<10.4f}")
    print(f"{'COG_y (mm)':<12} | {target_cg[1]:<25.4f} | {final_cg[1]:<25.4f} | {cg_y_error:<10.4f}")
    print(f"{'COG_z (mm)':<12} | {target_cg[2]:<25.4f} | {final_cg[2]:<25.4f} | {cg_z_error:<10.4f}")
    print(f"{'MOI_Ixx':<12} | {t_ixx:<25.2f} | {f_ixx:<25.2f} | {moi_ixx_error:<10.4f}")
    print(f"{'MOI_Iyy':<12} | {t_iyy:<25.2f} | {f_iyy:<25.2f} | {moi_iyy_error:<10.4f}")
    print(f"{'MOI_Izz':<12} | {t_izz:<25.2f} | {f_izz:<25.2f} | {moi_izz_error:<10.4f}")
    print(f"{'MOI_Ixy':<12} | {t_ixy:<25.2f} | {f_ixy:<25.2f} | {moi_ixy_error:<10.4f}")
    print(f"{'MOI_Ixz':<12} | {t_ixz:<25.2f} | {f_ixz:<25.2f} | {moi_ixz_error:<10.4f}")
    print(f"{'MOI_Iyz':<12} | {t_iyz:<25.2f} | {f_iyz:<25.2f} | {moi_iyz_error:<10.4f}")
...
if __name__ == '__main__':
    ...
    for gen in range(num_generations):
        fitness_scores = [calculate_fitness_score(ind, initial_mass, initial_cg, initial_moi, target_cg, target_moi, ballast_coords, ballast_mass) for ind in population]
        best_idx = np.argmin(fitness_scores)
        best_solution = population[best_idx]

        if (gen + 1) % 50 == 0:
            print(f"Generation {gen+1}, Best Fitness (Score): {fitness_scores[best_idx]:.4f}")

        # Check if the best solution is actually valid
        final_mass, final_cg, final_moi = calculate_combined_properties(initial_mass, initial_cg, initial_moi, ballast_coords, best_solution, ballast_mass)
        if count_violations(final_cg, final_moi, target_cg, target_moi) == 0:
    ...
    # --- Strategy C: Search with Simulated Annealing ---
    print(f"\n\n--- Strategy C: Searching All {num_locations} Locations with Simulated Annealing ---")
    best_solution_sa = run_simulated_annealing(initial_mass, initial_cg, initial_moi, target_cg, target_moi, ballast_coords, num_ballasts_to_add, ballast_mass)

    print("\nBest solution found by SA:")
    final_mass_sa, final_cg_sa, final_moi_sa = calculate_combined_properties(initial_mass, initial_cg, initial_moi, ballast_coords, best_solution_sa, ballast_mass)
    print_results_breakdown(final_mass_sa, final_cg_sa, final_moi_sa, target_mass, target_cg, target_moi)
